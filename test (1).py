...
def generate_desc(model, tokenizer, photo, max_length):
    ...
    for i in range(max_length):
        ...
        # Break if we can't find the word or we reach the end token
        ...
        in_text = 'start ' + ' '.join(result)
    
    # Join all words with proper spacing and remove start/end tokens
    final_caption = ' '.join(result).replace('start', '').replace('end', '').strip()
